...
GcsActorManager::GcsActorManager(boost::asio::io_context &io_context,
    ...
    : io_context_(io_context),
      ...
      gcs_actor_scheduling_strategy_(new GcsActorSchedulingStrategy(gcs_node_manager)) {}
...
void GcsActorManager::AsyncFlush(std::shared_ptr<GcsActor> actor,
  ...
  auto on_done = [actor, callback](Status status) {
    if (!status.ok()) {
      ...
      RAY_LOG(FATAL) << "Failed to update state to ALIVE for actor "
                     << actor->GetActorID();
    }
    if (callback) {
      callback();
...
void GcsActorManager::TryScheduling(std::shared_ptr<GcsActor> actor) {
  ...
  if (!node_id.IsNil()) {
    ...
    // The actor_table_data is just loaded from storage and the node id binded with
...
void GcsActorManager::LeaseWorker(std::shared_ptr<GcsActor> actor,
  ...
  RAY_LOG(DEBUG) << "Starting lease worker from node " << node->GetNodeId()
                 << " for actor " << actor->GetActorID();

  std::weak_ptr<GcsNode> weak_node(node);
  node->LeaseWorker(
      actor, [this, actor, weak_node](const Status &status, const ClientID &spillback_to,
                                      std::shared_ptr<GcsLeasedWorker> worker) {
        if (auto node = weak_node.lock()) {
          if (!status.ok()) {
            RAY_LOG(DEBUG) << "Failed to lease worker from node " << node->GetNodeId()
                           << " for actor " << actor->GetActorID()
...
void GcsActorManager::OnActorCreateSuccess(std::shared_ptr<GcsActor> actor,
                                           std::shared_ptr<GcsLeasedWorker> worker) {
  RAY_CHECK(actor && worker && actor->GetWorker() == nullptr);
  RAY_LOG(INFO) << "Succeed in creating actor " << actor->GetActorID() << " on worker "
                << worker->GetWorkerID() << " with node id " << worker->GetNodeID()
                << ", address " << worker->GetIpAddress() << ":" << worker->GetPort();
...
void GcsActorManager::CreateActor(const ray::rpc::CreateActorRequest &request,
  ...
  AsyncFlush(actor, [this, actor, callback] {
    // The actor is successfully persisted to the storage, so we could enqueue and create
    // this actor later and reply to the raylet.
    Enqueue(actor);
    callback(Status::OK());
  });
}
