...
class GcsNodeManager {
 public:
  ...
  /// \return true if node is not exist else false.
  bool AddNode(const rpc::GcsNodeInfo &node_info);
 ...
 protected:
  ...
  bool RemoveNode(const ClientID &node_id);
 ...
 private:
  rpc::ClientCallManager client_call_manager_;
  /// Alive gcs nodes.
  std::unordered_map<ClientID, std::shared_ptr<GcsNode>> nodes_;
